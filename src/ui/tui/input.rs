//! The input box: the keys it takes at the prompt, the line it yields, and
//! what the box carries between turns -- the history to recall, and the
//! questions the box answers in the machine's stead (the approval gate, the
//! secret prompt).
//!
//! Keys while a turn runs are the working module's: the loop is in a
//! different state then, and what the keys mean is different too (Enter
//! queues rather than submits, Ctrl-C cancels rather than clears).
