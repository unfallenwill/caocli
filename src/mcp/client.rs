...
impl Connection {
    ...
    /// Anthropic wire has a field for this (`is_error`), which the request
    /// builder sets from the leading `error: ` of the tool result — so a
    /// server's own failure is reported in the same shape every other tool's
    /// failure is, and a server that already said `error:` in its text is not
    /// told twice. The hub's caller reads this string for both the failure
    /// flag and the model-readable text; nothing here decides what to do with
    /// it.
...
fn initialize() -> Value {
    json!({
        ...
        "capabilities": {},
        "clientInfo": {"name": "caocli", "version": env!("CARGO_PKG_VERSION")},
    })
}
...
mod tests {
    ...
    async fn the_client_says_what_it_is_and_asks_for_the_newest_version() {
        ...
        assert!(first.contains("caocli"), "{first}");
