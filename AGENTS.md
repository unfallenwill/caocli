...
- **Terminal behavior only exists under a pty**, and a test's stdout is a pipe, so
  ...
  one-off run is what is wanted.
- `scripts/tui_smoke.py`'s live section (`SMOKE_LIVE=1`) is the only automated route
  to a turn *on screen*: the running status line, the approval gate, and the queue a
  line typed during a turn goes into. `scripts/pty_smoke.py` runs a live turn of its
  own — request, tool call, Ctrl-C — on the plain front end.
