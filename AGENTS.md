...
- Unit + integration tests: `cargo test`. A single test: `cargo test <pattern>`.
- The coverage gate needs `cargo-llvm-cov` and `llvm-tools-preview` installed; once they are, `cargo llvm-cov --fail-under-lines 90` enforces ≥ 90% lines.
...
- Before committing, run the four CI gates in order:
  ...
  3. `cargo test`
  4. `cargo llvm-cov --fail-under-lines 90`
