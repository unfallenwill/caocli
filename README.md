...
logs under `~/.caocli/sessions/`, one subdirectory per working
directory, resumable across runs and replayed byte-for-byte so the
backend's prefix cache keeps hitting. The file is the session's
execution trace, and its format is specified in
...
| `-c, --cont` | Continue the most recent session of this workspace (the file with the latest mtime under `~/.caocli/sessions/<escaped cwd>/`) |
| `--resume <ID>` | Resume a specific session by id. Looks in this workspace first, then walks every other workspace under `~/.caocli/sessions/`. Ids containing `/`, `..`, `\`, `%` or NUL are rejected — `--resume` cannot reach outside `sessions_dir`. |
| `--list` | List sessions of this workspace and exit (use `--migrate --all` to lift old v0 files to v1 — see `docs/session-format.md`) |
| `--list --all` | List sessions from every workspace on this machine, with their working directory |
| `--migrate <ID>` | Read a v0 session file and write a v1 sibling (`<id>-v1.jsonl`) beside it; the original is not touched. Combine with `--all` to migrate every v0 file across every workspace. |
